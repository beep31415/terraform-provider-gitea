...
func (a *NotificationAPIService) NotifyGetListExecute(r NotificationAPINotifyGetListRequest) ([]NotificationThread, *http.Response, error) {
	...
	if r.statusTypes != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "status-types", s.Index(i), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "status-types", t, "multi")
		}
	}
	if r.subjectType != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "subject-type", s.Index(i), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "subject-type", t, "multi")
		}
	}
	if r.since != nil {
